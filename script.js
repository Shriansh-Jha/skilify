...
// Default Courses Repository
    ...
    // Set this to the Web OAuth client ID from Google Cloud Console.
    ...
    function initializeGoogleLogin() {
      if (GOOGLE_CLIENT_ID.startsWith('YOUR_')) {
        ...
        if (signInButton) signInButton.innerHTML = '<span class="text-xs text-slate-400">Add Google client ID to enable sign-in</span>';
        return;
      }
      ...
      const savedUser = localStorage.getItem('skilify_google_user');
      if (savedUser) {
        try { showGoogleUser(JSON.parse(savedUser)); } catch { localStorage.removeItem('skilify_google_user'); }
      }
    }
    ...
    function handleGoogleCredential(response) {
      try {
        ...
        const user = { name: payload.name || 'Google user', picture: payload.picture || '', email: payload.email || '' };
        localStorage.setItem('skilify_google_user', JSON.stringify(user));
        showGoogleUser(user);
    ...
    function showGoogleUser(user) {
      const profile = document.getElementById('google-user-profile');
      const signInButton = document.getElementById('google-signin-button');
      const avatar = document.getElementById('google-user-avatar');
      const name = document.getElementById('google-user-name');
      if (!profile || !signInButton) return;
      avatar.src = user.picture;
      avatar.alt = `${user.name} profile photo`;
    ...
    function signOutGoogle() {
      localStorage.removeItem('skilify_google_user');
      if (window.google?.accounts?.id) google.accounts.id.disableAutoSelect();
      document.getElementById('google-user-profile')?.classList.add('hidden');
      document.getElementById('google-user-profile')?.classList.remove('flex');
    ...
    function saveProgressToStorage() {
      localStorage.setItem('skilify_progress', JSON.stringify(state.completedLessons));
    }
    ...
    function renderPracticeUI() {
      ...
      course.units.forEach((u, i) => {
        const opt = document.createElement('option');
        opt.value = i;
        opt.textContent = `Unit ${u.unitNumber || (i+1)}: ${u.unitTitle}`;
